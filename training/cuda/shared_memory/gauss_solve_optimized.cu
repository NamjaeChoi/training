...
constexpr unsigned int dim = 9;
constexpr unsigned int block_size = 126;

__global__ void gaussSolve(double * A, double * x, double * b)
{
    auto row = threadIdx.x;
    auto Row = threadIdx.y * dim + row;
    auto n = threadIdx.y + blockDim.y * blockIdx.x;

    __shared__ double shmem[dim * (dim + 1) * (block_size / dim)];

    double * mat = shmem;
    double * rhs = mat + dim * dim * blockDim.y;

    if (n < N)
    {
        for (unsigned int col = 0; col < dim; ++col)
            mat[Row + block_size * col] = A[row + dim * (col + dim * n)];
    ...
    {
        unsigned int R = threadIdx.y * dim + r;
        ...
        if (n < N)
        {
            if (row != r)
            {
                double m = mat[Row + block_size * r] / mat[R + block_size * r];

                rhs[Row] -= m * rhs[R];

                for (unsigned int col = r; col < dim; ++col)
                    mat[Row + block_size * col] -= m * mat[R + block_size * col];
            }
        }
    }

    if (n < N)
        x[row + dim * n] = rhs[Row] / mat[Row + block_size * row];
}
