...
public:
    ...
    __device__ double get() { return 1; }
};
...
public:
    ...
    __device__ double get() { return 2; }
};

__global__ void increment(Derived * value, double * result, unsigned int size)
{
...
int main(int argc, char * argv[])
{
    ...
    double * result;
    Derived * value;
    Derived * value_host = new Derived;

    cudaCheckError(cudaMalloc(&result, num_threads * sizeof(double)));
    cudaCheckError(cudaMalloc(&value, sizeof(Derived)));
    cudaCheckError(cudaMemcpy(value, value_host, sizeof(Derived), cudaMemcpyHostToDevice));
